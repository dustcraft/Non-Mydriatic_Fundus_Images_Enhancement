...
for k = 1 : ConvertFrameNum
    ...
    %mean RGB
    R_postprocessed_vs_origin_ms_ssim = msssim(R_recovery, R_border);
    G_postprocessed_vs_origin_ms_ssim = msssim(G_recovery, G_border);
    B_postprocessed_vs_origin_ms_ssim = msssim(B_recovery, B_border);
    
    mean_rgb_postprocessed_vs_origin_ms_ssim = (R_postprocessed_vs_origin_ms_ssim + G_postprocessed_vs_origin_ms_ssim + B_postprocessed_vs_origin_ms_ssim) / 3.0;
    
    R_postprocessed_vs_masked_ms_ssim = msssim(R_recovery, R_recorrected);
    G_postprocessed_vs_masked_ms_ssim = msssim(G_recovery, G_recorrected);
    B_postprocessed_vs_masked_ms_ssim = msssim(B_recovery, B_recorrected);
    
    mean_rgb_postprocessed_vs_masked_ms_ssim = (R_postprocessed_vs_masked_ms_ssim + G_postprocessed_vs_masked_ms_ssim + B_postprocessed_vs_masked_ms_ssim) / 3.0;
        
    %Y channel
    Y_postprocessed_vs_origin_ms_ssim = msssim(Y_recovery, Y_border);
    Y_postprocessed_vs_masked_ms_ssim = msssim(Y_recovery, Y_recorrected);
    
    M_Y(k,1) = Y_postprocessed_vs_origin_ms_ssim;
    M_Y1(k,1) = Y_postprocessed_vs_masked_ms_ssim;
    M_mean(k,1) = mean_rgb_postprocessed_vs_origin_ms_ssim;
    M_mean1(k,1) = mean_rgb_postprocessed_vs_masked_ms_ssim;
    image_name{k, 1} = name{1};

end

title = {'name', 'Y_channel:postprocessed_vs_origin', 'Y_channel:postprocessed_vs_masked', 'RGB_mean_value:postprocessed_vs_origin', 'RGB_mean_value:postprocessed_vs_masked'};
