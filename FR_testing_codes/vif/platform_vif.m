...
prefix = 'vif_matlab_recovery_benchmark_result';
...
for k = 1 : ConvertFrameNum
    ...
    %mean RGB
    R_postprocessed_vs_origin_vif = vifvec(R_recovery, R_border);
    G_postprocessed_vs_origin_vif = vifvec(G_recovery, G_border);
    B_postprocessed_vs_origin_vif = vifvec(B_recovery, B_border);
    
    mean_rgb_postprocessed_vs_origin_vif = (R_postprocessed_vs_origin_vif + G_postprocessed_vs_origin_vif + B_postprocessed_vs_origin_vif) / 3.0;
    
    R_postprocessed_vs_masked_vif = vifvec(R_recovery, R_recorrected);
    G_postprocessed_vs_masked_vif = vifvec(G_recovery, G_recorrected);
    B_postprocessed_vs_masked_vif = vifvec(B_recovery, B_recorrected);
    
    mean_rgb_postprocessed_vs_masked_vif = (R_postprocessed_vs_masked_vif + G_postprocessed_vs_masked_vif + B_postprocessed_vs_masked_vif) / 3.0;
        
    %Y channel
    Y_postprocessed_vs_origin_vif = vifvec(Y_recovery, Y_border);
    Y_postprocessed_vs_masked_vif = vifvec(Y_recovery, Y_recorrected);
    
    M_Y(k,1) = Y_postprocessed_vs_origin_vif;
    M_Y1(k,1) = Y_postprocessed_vs_masked_vif;
    M_mean(k,1) = mean_rgb_postprocessed_vs_origin_vif;
    M_mean1(k,1) = mean_rgb_postprocessed_vs_masked_vif;
