...
for k = 1 : ConvertFrameNum
    ...
    %mean RGB
    R_postprocessed_biqi = biqi(R_recovery);
    G_postprocessed_biqi = biqi(G_recovery);
    B_postprocessed_biqi = biqi(B_recovery);
    
    mean_rgb_postprocessed_biqi = (R_postprocessed_biqi + G_postprocessed_biqi + B_postprocessed_biqi) / 3.0;
       
    %Y channel
    Y_postprocessed_biqi = biqi(Y_recovery);
    
    M_Y(k,1) = Y_postprocessed_biqi;
    M_mean(k,1) = mean_rgb_postprocessed_biqi;
    image_name{k, 1} = name{1};

end

title = {'name', 'Y_channel', 'RGB_mean_value'};
temp = [image_name, num2cell(M_Y), num2cell(M_mean)];
